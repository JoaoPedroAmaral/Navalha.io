...
export default function SchedulePage() {
  ...
  const [localEdits, setLocalEdits] = useState<
    Record<string, Partial<WorkSchedule>>
  >({});
  ...
  const effectiveBarberId =
    selectedBarberId || (barbers.length > 0 ? barbers[0].id : "");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="max-w-xs">
        <Select value={effectiveBarberId} onValueChange={handleBarberChange}>
          <SelectTrigger>
      ...
      {!effectiveBarberId ? (
        <div className="bg-white rounded-xl border p-12 text-center shadow-sm">
          ...
          <p className="text-gray-500 font-medium">
            Selecione um Funcionario para configurar a agenda
          </p>
        </div>
      ) : isLoading ? (
